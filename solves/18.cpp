...
{
  // input
  ulong T,L,X,Y,Q; cin >> T >> L >> X >> Y >> Q;

  // main
  // x = -L/2 * sin(t/T * 2PI)
  // y = L/2 + -L/2 * cos(t/T * 2PI)
  rep (q, Q) {
    double E; cin >> E;
    double y = -1.0 * L/2.0 * sin(E/(double)T * 2.0 * PI);
    double z = L/2.0  - L/2.0 * cos(E/(double)T * 2.0 * PI);
    double tmp = sqrt(pow((double)X, 2.0) + pow((double)Y - y, 2.0));
    double theta = z / tmp; 
    double angle = atan(theta) / (2.0 * PI) * 360.0;
    cout << fixed << setprecision(12)  << angle << endl;
  }
