...
int main(void)
{
  // input
  llong N; cin >> N;
  vector<llong> typs(3); rep(ix, 3) cin >> typs[ix];
  sort(typs.begin(), typs.end(), greater<llong>());
  llong A = typs[0], B = typs[1], C = typs[2];

  // main
  llong _min = LONG_LONG_MAX;
  // <= O(N^2)
  for (llong a = 0; a <= 9999; ++a) {
    for (llong b = 0; a + b <= 9999; ++b) {
      auto rest = (N - (a * A + b * B));
      if (rest < 0) break;
      if (rest % C == 0) _min = min(_min, a + b + rest/C);
    }
  }
