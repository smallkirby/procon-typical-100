...
int main(void)
{
  // input
  llong N; cin >> N;
  vector<llong> t(N),x(N),y(N);
  rep(ix,N) cin >> t[ix] >> x[ix] >> y[ix];

  // main
  rep(ix, N) {
    const llong prev_x = ix == 0 ? 0 : x[ix-1];
    const llong prev_y = ix == 0 ? 0 : y[ix-1];
    const llong prev_t = ix == 0 ? 0 : t[ix-1];
    const llong diff = abs(prev_x - x[ix]) + abs(prev_y - y[ix]);
    if (diff > (t[ix] - prev_t) || ((t[ix] - prev_t) - diff) % 2 == 1) {
      cout << "No" << endl;
      return 0;
    }
  }
